...
describe('answer endpoints', function() {
    describe('integration tests', function() {
        ...
        it('(put /question/questionId/answer/open) should update open answer', async function() {
            ...
            const UpdateRes = await requester.put(`/api/answer/question/${questionId}/open/${answer._id}`).set({ Authorization: `Bearer ${jwt}` }).send(updatedAnswer)
        ...
        it('(put /question/questionId/answer/open) should update open answer of other user', async function() {
            ...
            const UpdateRes = await requester.put(`/api/answer/question/${questionId}/open/${answer._id}`).set({ Authorization: `Bearer ${jwt2}` }).send(updatedAnswer)
        ...
        it('(delete /question/questionId/answer/open/answerid) should delete open answer', async function() {
            ...
            const UpdateRes = await requester.delete(`/api/answer/question/${questionId}/open/${answer._id}`).set({ Authorization: `Bearer ${jwt}` }).send(updatedAnswer)
            expect(UpdateRes).to.have.status(204)
        ...
        it('(delete /question/questionId/answer/open/answerid) should not delete open answer of other user', async function() {
            ...
            const UpdateRes = await requester.delete(`/api/answer/question/${questionId}/open/${answer._id}`).set({ Authorization: `Bearer ${jwt2}` }).send()
            expect(UpdateRes).to.have.status(401)
        ...
        it('(put /question/questionId/answer/multi) should update multi answer', async function() {
            ...
            const UpdateRes = await requester.put(`/api/answer/question/${questionId}/multi/${answer._id}`).set({ Authorization: `Bearer ${jwt}` }).send(updatedAnswer)
            expect(UpdateRes).to.have.status(204)
            
            
            const upAnswer = await MultiAnswer.findOne({_id: answerRes.body._id})
            expect(upAnswer).to.have.property('content', updatedAnswer.content)
            expect(upAnswer).to.have.property('selectedChoice', updatedAnswer.selectedChoice)
        ...
        it('(put /question/questionId/answer/multi) should not update multi answer of other user', async function() {
            ...
            const UpdateRes = await requester.put(`/api/answer/question/${questionId}/multi/${answer._id}`).set({ Authorization: `Bearer ${jwt2}` }).send(updatedAnswer)
            expect(UpdateRes).to.have.status(401)
            
            
            const upAnswer = await MultiAnswer.findOne({_id: answerRes.body._id})
            expect(upAnswer).to.have.property('content', testAnswer.content)
            expect(upAnswer).to.have.property('selectedChoice', testAnswer.selectedChoice)
        ...
        it('(delete /question/questionId/answer/multi) should delete multi answer', async function() {
            ...
            const UpdateRes = await requester.delete(`/api/answer/question/${questionId}/multi/${answer._id}`).set({ Authorization: `Bearer ${jwt}` }).send()
            expect(UpdateRes).to.have.status(204)
        ...
        it('(delete /question/questionId/answer/multi) should not delete multi answer of other user', async function() {
            ...
            const UpdateRes = await requester.delete(`/api/answer/question/${questionId}/multi/${answer._id}`).set({ Authorization: `Bearer ${jwt2}` }).send()
            expect(UpdateRes).to.have.status(401)
        ...
        it('(put /question/questionId/answer/scale) should update scale answer', async function() {
            ...
            const UpdateRes = await requester.put(`/api/answer/question/${questionId}/scale/${answer._id}`).set({ Authorization: `Bearer ${jwt}` }).send(updatedAnswer)
            expect(UpdateRes).to.have.status(204)
            
            
            const upAnswer = await ScaleAnswer.findOne({_id: answerRes.body._id})
            expect(upAnswer).to.have.property('content', updatedAnswer.content)
            expect(upAnswer).to.have.property('selectedScale', updatedAnswer.selectedScale)
        })
        ...
        it('(put /question/questionId/answer/scale) should not update scale answer of ohter user', async function() {
            ...
            const UpdateRes = await requester.put(`/api/answer/question/${questionId}/scale/${answer._id}`).set({ Authorization: `Bearer ${jwt2}` }).send(updatedAnswer)
            expect(UpdateRes).to.have.status(401)
            
            
            const upAnswer = await ScaleAnswer.findOne({_id: answerRes.body._id})
            expect(upAnswer).to.have.property('content', testAnswer.content)
            expect(upAnswer).to.have.property('selectedScale', testAnswer.selectedScale)
        })
        ...
        it('(delete /question/questionId/answer/scale) should delete scale answer', async function() {
            ...
            const UpdateRes = await requester.delete(`/api/answer/question/${questionId}/scale/${answer._id}`).set({ Authorization: `Bearer ${jwt}` }).send(updatedAnswer)
            expect(UpdateRes).to.have.status(204)
        ...
        it('(delete /question/questionId/answer/scale) should not delete scale answer of other user', async function() {
            ...
            const UpdateRes = await requester.delete(`/api/answer/question/${questionId}/scale/${answer._id}`).set({ Authorization: `Bearer ${jwt2}` }).send(updatedAnswer)
            expect(UpdateRes).to.have.status(401)
        ...
        it('(put /question/questionId/answer/percentage) should update percentage answer', async function() {
            ...
            const UpdateRes = await requester.put(`/api/answer/question/${questionId}/percentage/${answer._id}`).set({ Authorization: `Bearer ${jwt}` }).send(updatedAnswer)
            expect(UpdateRes).to.have.status(204)
            
            
            const upAnswer = await PercentageAnswer.findOne({_id: answerRes.body._id})
            expect(upAnswer).to.have.property('content', updatedAnswer.content)
        ...
        it('(put /question/questionId/answer/percentage) should not update percentage answer of other user', async function() {
            ...
            const UpdateRes = await requester.put(`/api/answer/question/${questionId}/percentage/${answer._id}`).set({ Authorization: `Bearer ${jwt2}` }).send(updatedAnswer)
            expect(UpdateRes).to.have.status(401)
        ...
        it('(delete /question/questionId/answer/percentage) should delete percentage answer', async function() {
            ...
            const UpdateRes = await requester.delete(`/api/answer/question/${questionId}/percentage/${answer._id}`).set({ Authorization: `Bearer ${jwt}` }).send()
            expect(UpdateRes).to.have.status(204)
        ...
        it('(delete /question/questionId/answer/percentage) should not delete percentage answer of other user', async function() {
            ...
            const UpdateRes = await requester.delete(`/api/answer/question/${questionId}/percentage/${answer._id}`).set({ Authorization: `Bearer ${jwt2}` }).send()
            expect(UpdateRes).to.have.status(401)
